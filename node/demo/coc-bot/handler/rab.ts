...
export async function handleRAB(
  client: TRPGClient,
  ...
  try {
    ...
    if (!isValidString(skillName)) {
      ...
      client.sendReplyGroupMessage(
        payload,
        `输入不合法, 格式应为 /rab [技能名]( [专长]) (属性值), 请重新检查输入: ${message}`
      );
      return;
    }
    ...
    console.log(`奖励投骰结果是: ${bonusResultList}`);

    const bonusPoint = Math.min(...bonusResultList);
    console.log(`选取奖励点数: ${bonusPoint}`);
    ...
    finallyRollValue = Math.min(diceRes.dice_result, finallyRollValue);
    ...
    client.sendReplyGroupMessage(
      payload,
      `B${count}=${diceRes.dice_result}[奖励骰: ${bonusResultList.join(
        ' '
